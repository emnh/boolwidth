...
public class AggregateDemo extends Display {
	...
	private void initDataGroups() {
		...
		this.m_vis.setValue(NODES, null, VisualItem.SHAPE, new Integer(
		...
		AggregateItem saitem = (AggregateItem) at.addItem();
		AggregateItem saitem2 = (AggregateItem) at.addItem();
		for (int i = 0; i < 3; ++i) {
			...
			if (i > 0) {
				saitem.addItem(aitem);
			}
			if (i < 2) {
				saitem2.addItem(aitem);
			}
		}
...
class AggregateLayout extends Layout {
	...
	public void run(double frac) {
		...
		AggregateTable aggr = (AggregateTable) this.m_vis
				.getGroup(this.m_group);
		...
		Iterator<?> aggrs = this.m_vis.visibleItems(this.m_group);
		while (aggrs.hasNext()) {
			...
			// compute convex hull
			double[] nhull = GraphicsLib.convexHull(this.m_pts, idx);
