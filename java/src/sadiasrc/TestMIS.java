...
public class TestMIS {
		...
		public static void main(String[] args) {
			...
			String fileName =  ControlInput.GRAPHLIB+"DIMACS_PP/brock200_4.graph.mis.sadia";
			String dirName = ControlInput.GRAPHLIB + "DIMACS_PP/";
			...
			long  end,end1;//end time for an ordering 1cuk-pp		&	552.521	&	0.005	&	552.526	&	14.9	&	1.002	&	20.15	\\
...
//				IndexGraph G = IndexGraph.Grid(gridsize);
				...
				//use if single file as input
				G =	cio.getTestGraph(fileName, G);
			names=fileName.split("/");

				//Use if directory as input
...
////////				
				...
				if(G.numVertices()>0 && G.numVertices()<=2400){
				...
				/*	start=System.currentTimeMillis();
					...
					System.out.println("Time taken: "+df.format( (end-start))+ " ms");
					
					result += "Symdiff "+ " "+ df.format( (end-start))+ " ";
					start=System.currentTimeMillis();
					UBfromCP=caterpiller.getLinearBooleanWidth(OrderedSequence, G);
					end=System.currentTimeMillis();
					System.out.println("UB : "+df.format(Math.log(UBfromCP)/(Math.log(2)))+" from symdiff Time taken: "+ (end-start)+ " ms");
					
					result += df.format(Math.log(UBfromCP)/(Math.log(2)))+ " ";//+ (end-start)/1000+ " ";
					...
					result += "MIS"+ " "+ (maxis+taken_in_MIS)+" "+df.format((end-start))+ " ";*/
...
//				
				...
				//	System.out.println("Ordered Sequence: "+OrderedSequence);	
					...
					result += "Grwnghbr WS "+ " "+ df.format((end-start))+ " ";
//				
...
//					
				...
				/*	start=System.currentTimeMillis();
					...
					System.out.println("Time taken: "+ (end-start)+ " ms");
					System.out.println("UB : "+df.format(Math.log(chooser.getUB())/(Math.log(2)))+" from Greedy Time taken: "+ (end-start)+ " ms");
					
					result += "Greedy"+ " "+ df.format((end-start)/1000)+ " "+ "UB"+ " "+df.format(Math.log(chooser.getUB())/(Math.log(2))) +" ";
			...
			/*		start=System.currentTimeMillis();
				...
				/*	start=System.currentTimeMillis();
					...
					result += "Ordering taken "+ " "+df.format((end-start))+ " ";	
				...
				/*	start=System.currentTimeMillis();
					...
					result += df.format(Math.log(UBfromCP)/(Math.log(2)))+ " "+ (end-start)+ " ";*/
		...
		/*			start=System.currentTimeMillis();
					...
					choosen = BasicGraphAlgorithms.BFS(G,G.vertices().iterator().next());
					choosen = BasicGraphAlgorithms.BFS(G,choosen);
					maxis =IndependentSet.linIS(G,choosen);
					end=System.currentTimeMillis();
					System.out.println("Independent Set : "+ maxis+ " time taken : "+ (end-start)+ " ms");
					
					result += "DP"+ "  "+df.format((end-start))+ " ";	*/
				...
				File newfile = new File(ControlInput.GRAPHLIB + "output/"+"CH9_TWLIB_O2.txt");
